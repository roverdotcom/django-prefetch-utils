...
class MultiColumnFKTests(TestCase):
    ...
    def test_reverse_query_returns_correct_result(self):
        ...
        Membership.objects.create(
            membership_country_id=self.soviet_union.id, person_id=self.bob.id, group_id=self.republican.id
        )
    ...
    def test_query_filters_correctly(self):
        ...
        Membership.objects.create(
            membership_country_id=self.soviet_union.id, person_id=self.george.id, group_id=self.cia.id
        )

        self.assertQuerysetEqual(
            Membership.objects.filter(person__name__contains="o"), [self.bob.id], attrgetter("person_id")
        )
    ...
    def test_reverse_query_filters_correctly(self):
        ...
        self.assertQuerysetEqual(
            Person.objects.filter(membership__date_joined__gte=timemark), ["Jim"], attrgetter("name")
        )
    ...
    def test_forward_in_lookup_filters_correctly(self):
        ...
        self.assertQuerysetEqual(
            Membership.objects.filter(person__in=[self.george, self.jim]), [self.jim.id], attrgetter("person_id")
        )
        self.assertQuerysetEqual(
    ...
    def test_translations(self):
        ...
        self.assertEqual(
            list(Article.objects.filter(active_translation__abstract=None, active_translation__pk__isnull=False)), [a1]
        )
    ...
    def test_inheritance(self):
        ...
        with self.assertNumQueries(1):
            self.assertEqual(
                NewsArticle.objects.select_related("active_translation")[0].active_translation.title, "foo"
            )
    ...
    def test_isnull_lookup(self):
        ...
        Membership.objects.create(membership_country=self.usa, person=self.bob, group=self.cia)
        self.assertQuerysetEqual(
            Membership.objects.filter(group__isnull=True), ["<Membership: Bob is a member of NULL>"], transform=repr
        )
        self.assertQuerysetEqual(
            Membership.objects.filter(group__isnull=False), ["<Membership: Bob is a member of CIA>"], transform=repr
        )
    ...
    def test_check_composite_foreign_object(self):
        ...
        class Child(models.Model):
            ...
            parent = ForeignObject(
                Parent, on_delete=models.SET_NULL, from_fields=("a", "b"), to_fields=("a", "b"), related_name="children"
            )
