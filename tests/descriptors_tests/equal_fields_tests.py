...
class EqualFieldsDescriptorWithOneJoinTests(
    ...
    def setUpTestData(cls):
        cls.first_book = BookWithYearlyBios.objects.create(
            title="Book One",
            published_year=1900
        )
        cls.second_book = BookWithYearlyBios.objects.create(
            title="Book Two",
            published_year=1980
        )
        ...
        cls.first_bio = YearlyBio.objects.create(
            author=cls.author,
            year=cls.first_book.published_year
        )
        cls.second_bio = YearlyBio.objects.create(
            author=cls.author2,
            year=cls.second_book.published_year
        )
...
class EqualFieldsDescriptorWithMultipleJoinsTests(
        GenericQuerySetDescriptorTestCaseMixin,
        TestCase):
    descriptor_class = EqualFieldsDescriptor
    attr = 'ones'
...
class EqualFieldsDescriptorWithCommonTests(TestCase):
    ...
    def test_preprocess_join_fields_single_string(self):
        self.assertEqual(
            self.descriptor.preprocess_join_fields('a'),
            [('a', 'a')]
        )

    def test_preprocess_join_fields_list_of_strings(self):
        self.assertEqual(
            self.descriptor.preprocess_join_fields(['a', 'b']),
            [('a', 'a'), ('b', 'b')]
        )

    def test_preprocess_join_fields_list_of_tuples(self):
        self.assertEqual(
            self.descriptor.preprocess_join_fields([
                ('a', 'c'),
                ('b', 'd'),
            ]),
            [('a', 'c'), ('b', 'd')]
